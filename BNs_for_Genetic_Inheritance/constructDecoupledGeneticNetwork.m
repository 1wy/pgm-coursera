...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
% child copy given genotype
childCopyGivenPar = childCopyGivenParentalsFactor(numAlleles, 3, 1, 2);
% child copy without parents, so it is alleleFreqs
childCopyGivenFreq = childCopyGivenFreqsFactor(alleleFreqs, 2);
% genotype given parent
phenotypeGivenCopy = phenotypeGivenCopiesFactor(alphaList, numAlleles, 2, 3, 1);

% genotypeFactorPar = genotypeGivenParentsGenotypesFactor(numAlleles, 3, 1, 2);
for i = 1:numPeople
   if (pedigree.parents(i,1) == 0) && (pedigree.parents(i,2) == 0)
       tmp_factor_copy1 = childCopyGivenFreq;
       tmp_factor_copy1.var = i;
       tmp_factor_copy2 = childCopyGivenFreq;
       tmp_factor_copy2.var = i + numPeople;
   else
       tmp_factor_copy1 = childCopyGivenPar;
       tmp_factor_copy1.var = [i, pedigree.parents(i,1), pedigree.parents(i,1)+numPeople];
       
       tmp_factor_copy2 = childCopyGivenPar;
       tmp_factor_copy2.var = [i+numPeople, pedigree.parents(i,2), pedigree.parents(i,2)+numPeople];
   end
   factorList(i) = tmp_factor_copy1;
   factorList(i+numPeople) = tmp_factor_copy2;
   
   tmp_factor = phenotypeGivenCopy;
   tmp_factor.var = [i+2*numPeople, i, i+numPeople];
   factorList(i+2*numPeople) = tmp_factor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  