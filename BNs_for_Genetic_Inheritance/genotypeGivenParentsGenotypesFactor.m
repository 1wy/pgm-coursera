...
card = size(genotypesToAlleles,1);
genotypeFactor.card = [card, card, card];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));

[x, y] = meshgrid(1:card, 1:card);


comb = [y(:), x(:)];
table = zeros(length(comb), card);
for i = 1:length(comb)
   par1 = genotypesToAlleles(comb(i,1), :);
   par2 = genotypesToAlleles(comb(i,2), :);
   [par1, par2] = meshgrid(par1, par2);
   child = [par1(:), par2(:)];

   for j = 1:4
       p = min(child(j,:));
       q = max(child(j,:));
%        map the gene pair(p,q) to the index in the genotypesToAlleles
       idxchild = card - (numAlleles-p+1)*(numAlleles-p)/2 - (numAlleles-q); 
       table(i, idxchild) = table(i, idxchild) + 0.25;
   end

   
end
table = table';
genotypeFactor.val = table(:)';
