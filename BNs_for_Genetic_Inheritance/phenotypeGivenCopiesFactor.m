...
[x, y] = meshgrid(1:numAlleles, 1:numAlleles);
comb = [x(:), y(:)];
table = zeros(length(comb),1);
for i = 1:length(table)
   p = min(comb(i,:));
   q = max(comb(i,:));
   idx = length(genotypesToAlleles) - (numAlleles-p+1)*(numAlleles-p)/2 - (numAlleles-q); 
   table(i) = alphaList(idx);
end
