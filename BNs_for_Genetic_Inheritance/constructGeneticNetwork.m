...
phenotypeFactorAlpha = phenotypeGivenGenotypeFactor(alphaList, 3, 1);
% genotype given AlleleFreqs
genotypeFactor = genotypeGivenAlleleFreqsFactor(alleleFreqs, 3);
...
for i = 1:numPeople
   if (pedigree.parents(i,1) == 0) && (pedigree.parents(i,2) == 0)
       tmp_factor = genotypeFactor;
       tmp_factor.var = i;
   else
       tmp_factor = genotypeFactorPar;
       tmp_factor.var = [i, pedigree.parents(i,:)];
   end
   factorList(i) = tmp_factor;
   tmp_factor = phenotypeFactorAlpha;
   tmp_factor.var = [i+numPeople, i];
   factorList(i+numPeople) = tmp_factor;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
end