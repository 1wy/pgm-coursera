...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    if (doLogZ)
        %%% YOUR CODE HERE:
%         logZ = 0; % remove this
        % find a valid edge and ouput the potensial of the sepset variables.
        % then sum them we can get the results. 
        [i, j] = find(P.edges, 1);
        facprod = FactorProduct(unnormalizedMessages(i,j), unnormalizedMessages(j,i));
        logZ = log(sum(facprod.val));
    else
        logZ = 0;
    end
