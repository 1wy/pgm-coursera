...
for c = 1:3
    ...
    for i = 1:I
        NPose = length(datasetTest.actionData(i).marg_ind);
        F = repmat(struct('var',[],'card',[],'val',[]), 1, 2*NPose);
        F(1).var = 1;    F(1).card = [K];    F(1).val = log(P(c).c);
        logTranMax = log(reshape(P(c).transMatrix',1,9));
        ...
        for j = 1:NPose
            F(j+NPose).var = [j];    F(j+NPose).card = [K];    F(j+NPose).val = logEmissionProb(datasetTest.actionData(i).marg_ind(j),:);
        end
